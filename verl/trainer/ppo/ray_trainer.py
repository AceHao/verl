...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            teacher_ids = test_output_gen_batch.batch["teacher_response"]
            ...
            def safe_rouge_score(ref, cand):
                try:
                    return scorer.score(ref, cand)['rougeL'].fmeasure
                except RecursionError:
                    print(f"Warning: RecursionError in ROUGE scoring, using fallback score 0.0 for ref length {len(ref)} and cand length {len(cand)}")
                    return 0.0
                except Exception as e:
                    print(f"Warning: Error in ROUGE scoring: {e}, using fallback score 0.0")
                    return 0.0
            
            scores = [safe_rouge_score(ref, cand) for ref, cand in zip(teacher_texts, output_texts)]
        ...
        metric_dict["val/rouge-L/mean"] = np.mean(sample_scores)
