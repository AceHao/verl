...
class DataParallelPPOCritic(BasePPOCritic):
    ...
    # NOTE: _checkpointed_forward removed as it is no longer needed with use_reentrant=False
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    # We use use_reentrant=False which is safer for FSDP/MoE.
                    # determinism_check="none" allows MoE routing to vary between passes without crashing.
                    # preserve_rng_state=True (default) keeps the random seed consistent to minimize jitter.
                    
                    student_vpreds = torch_checkpoint(
                        self._forward_micro_batch,
                        data, False,  # micro_batch, compute_teacher
                        use_reentrant=False,
                    ...
                    teacher_vpreds = torch_checkpoint(
                        self._forward_micro_batch,
                        data, True,   # micro_batch, compute_teacher
                        use_reentrant=False,
                        determinism_check="none",
                        preserve_rng_state=True
                    )
